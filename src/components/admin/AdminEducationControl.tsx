...
import { 
  GraduationCap, Users, Upload, Download, School, 
  Search, RefreshCw
} from 'lucide-react'
import { companyService } from '@/services/companyService'
import type { CompanyResponse, Employee, EmployeeUploadResponse } from '@/services/companyService'

export function AdminEducationControl() {
  const [schools, setSchools] = useState<CompanyResponse[]>([])
  const [selectedSchool, setSelectedSchool] = useState<CompanyResponse | null>(null)
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const data = await companyService.listCompaniesByProduct('SINAPSE_360_EDUCACAO')
      setSchools(data)
  ...
  const handleSelectSchool = async (school: CompanyResponse) => {
    setSelectedSchool(school)
    setUploadResult(null)
    setSelectedFile(null)
    try {
      const emp = await companyService.listEmployees(school.id)
      setEmployees(emp)
    } catch (err) {
      console.error('Erro ao carregar colaboradores da escola:', err)
    }
  }
  ...
  const handleUpload = async () => {
    ...
    setUploadResult(null)
    try {
      const res = await companyService.uploadEmployeesCsv(selectedSchool.id, selectedFile)
      setUploadResult(res)
      if (res.totalImported > 0) {
        const emp = await companyService.listEmployees(selectedSchool.id)
        setEmployees(emp)
        loadData()
      }
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      {/* Top Banner */}
      <div className="bg-gradient-to-r from-orange-600 via-amber-600 to-purple-800 rounded-3xl p-6 sm:p-8 text-white shadow-lg flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-2xl font-bold">Sinapse 360° Educação</h2>
              <Badge className="bg-white/20 text-white border-white/30 text-xs">Módulo Escolar</Badge>
            </div>
            <p className="text-white/80 text-sm mt-1">
              Controle de escolas, corpo docente, suporte a alunos neurodivergentes e clima educacional.
            </p>
          </div>
        </div>
      ...
      {/* Grid: Lista de Escolas + Painel de Gestão da Escola Selecionada */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {/* Coluna 1: Lista de Escolas */}
        <Card className="rounded-3xl border-gray-100 shadow-sm bg-white">
          ...
          <CardContent className="p-6 pt-2 space-y-2 max-h-[500px] overflow-y-auto">
            {schools.length === 0 ? (
              <p className="text-xs text-gray-400 text-center py-8">Nenhuma instituição de ensino cadastrada.</p>
            ) : (
              schools.map(s => (
                <div 
                  key={s.id}
        ...
        {/* Coluna 2 e 3: Gestão dos Docentes / Colaboradores da Escola */}
        <div className="lg:col-span-2 space-y-6">
          {selectedSchool ? (
            <>
              {/* Card de Upload Rápido para a Escola Selecionada */}
              <Card className="rounded-3xl border-gray-100 shadow-sm bg-white">
                ...
                <CardContent className="p-6 pt-3 space-y-4">
                  <div className="flex flex-col sm:flex-row items-center gap-3">
                    <Input 
                      ...
                      className="rounded-xl text-xs h-10 file:mr-3 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-orange-50 file:text-orange-700"
                    />
                    <Button 
                      onClick={handleUpload}
                      disabled={!selectedFile || isUploading}
                      className="w-full sm:w-auto bg-orange-500 hover:bg-orange-600 text-white rounded-xl text-xs font-semibold h-10 px-5 shrink-0"
                    >
              ...
              {/* Tabela de Colaboradores / Professores */}
              <Card className="rounded-3xl border-gray-100 shadow-sm bg-white">
                <CardHeader className="p-6 pb-3">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                      <CardTitle className="text-base font-bold text-gray-900 flex items-center gap-2">
                        ...
                        Equipe Escolar ({employees.length})
                      </CardTitle>
                      <CardDescription className="text-xs text-gray-500">
                        Docentes, psicopedagogos e funcionários com acesso liberado.
                      </CardDescription>
                    </div>
                ...
                <CardContent className="p-6 pt-0">
                  {filteredEmployees.length === 0 ? (
                    <p className="text-xs text-gray-400 text-center py-8">Nenhum colaborador encontrado para esta escola.</p>
                  ) : (
                      ...
                      <table className="w-full text-left text-xs text-gray-600">
                        <thead className="bg-gray-50 uppercase text-[10px] text-gray-400 font-semibold border-b border-gray-100">
                          <tr>
                            ...
                            <th className="py-2.5 px-3">Setor/Escola</th>
                        ...
                        <tbody className="divide-y divide-gray-100">
                          {filteredEmployees.map(emp => (
                            <tr key={emp.id} className="hover:bg-orange-50/30 transition-colors">
                              ...
                              <td className="py-2.5 px-3">{emp.department || emp.school || '-'}</td>
            ...
            <div className="text-center py-16 bg-white rounded-3xl border border-gray-100 text-gray-400">
              ...
              <p className="text-sm font-medium">Selecione uma escola ao lado para gerenciar sua equipe.</p>
            </div>
          )}
        </div>
      </div>
